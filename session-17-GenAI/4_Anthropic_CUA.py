"""
Computer-Use Agent Demo with Anthropic Claude
...
pip install playwright anthropic python-dotenv nest_asyncio
...
# For Jupyter display (comment out if running as .py file)
try:
...
if api_key:
    ...
    print("[ERROR] No API key provided! Add ANTHROPIC_API_KEY to .env file")
    exit()
...
def show_clickables(clickables: list):
    ...
    if IN_NOTEBOOK:
        ...
        print("\nClickable Elements:")
        for c in clickables[:15]:
            print(f"  [{c['idx']}] {c['text'][:40]} -> {c['href'][:50]}")

print("[OK] Helper functions defined!")

# =============================================================================
# LLM Decision Maker (Anthropic Claude)
# =============================================================================
def llm_decide(state: str, clickables: list, goal: str, current_url: str) -> dict:
    """
    Use Claude to decide the next action based on page state.
    """
...
async def run_agent():
    ...
    async with async_playwright() as p:
        ...
        for step in range(1, max_steps + 1):
            ...
            # DECIDE: Ask Claude for next action
